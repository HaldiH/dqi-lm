...
def process_file_id(file_id, directory_path):
    ...
    if (
        os.path.exists(speaker_map_file)
        and os.path.exists(descr_file)
        and os.path.exists(speeches_file)
    ):
        try:
            speaker_map = pd.read_csv(
                speaker_map_file, sep="|", dtype=str, encoding="latin1"
            )
            ...
            logging.warning(
                f"Encoding error in files for {file_id}, attempting with 'ISO-8859-1' encoding."
            )
            speaker_map = pd.read_csv(
                speaker_map_file, sep="|", dtype=str, encoding="ISO-8859-1"
            )
        ...
        final_merged_data = pd.merge(
            merged_data,
            speaker_map,
            on=["speech_id", "lastname", "chamber", "gender", "state"],
            how="left",
        )
...
def merge_datasets(directory_path, output_file_path):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(process_file_id, f"{i:03}", directory_path)
    ...
    concatenated_data["date"] = pd.to_datetime(
        concatenated_data["date"], format="%Y%m%d"
    ).dt.strftime("%m/%d/%Y")
    concatenated_data["legislature"] = concatenated_data["chamber"].apply(
        lambda x: "Senate" if x == "S" else "House"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Merge US debate datasets.")
    parser.add_argument(
    ...
    output_file_path = config["data"]["merged_debates_path"]
